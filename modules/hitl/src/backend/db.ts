...
export default class HitlDb {
  ...
  renderHitlCompatibleMessage = async ({ payload, channel, botId }) => {
    // Convert singl choise to HITL-compatible format
    ...
    // Render element for HITL as channel-web element
    if (channel !== 'web' && payload.contentId) {
      ...
      const raw_message = await contentTypeRenderer.renderElement({ ...payload.elementData, typing: false }, 'web')[0]
      return raw_message
    }
